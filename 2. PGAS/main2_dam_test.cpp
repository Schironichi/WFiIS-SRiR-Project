...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        if (x > 0)
        {
            ...
            // Global P array
            gp_ptr = upcxx::new_array<double>((rows - i) * (rows - i));
            cout << "\nSCATTER P -> mat" << rank << "\n";
            ...
            upcxx::broadcast(&gp_ptr, rows - i, 0).wait();
